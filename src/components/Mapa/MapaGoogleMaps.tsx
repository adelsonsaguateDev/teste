...
export default function MapaGoogleMaps() {
  ...
  }, [userPosition, salaEncontrada]);

  // --- LÓGICA DE PESQUISA ---
  const procurarSala = () => {
    const sala = salasExame.find(
      (s) => s.codigoCandidato === pesquisa.trim() || s.nome.toLowerCase() === pesquisa.trim().toLowerCase()
    );
  ...
  const renderMap = () => {
    ...
    return (
      <GoogleMap mapContainerStyle={containerStyle} center={userPosition || salasExame[0].pos} zoom={16} options={mapOptions}>
        ...
        {salaEncontrada && <MarkerF position={salaEncontrada.pos} title={salaEncontrada.nome} />}
      </GoogleMap>
    );
  }
  ...
  return (
    <LayoutInterno>
      <div className="h-screen flex flex-col">
        ...
        <AnimatePresence>
          {salaEncontrada && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-200">
              <div className="max-w-7xl mx-auto p-4">
                <div className="bg-white rounded-2xl p-6 shadow-lg border border-green-200">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-green-100 rounded-2xl flex items-center justify-center"><MapPin className="w-6 h-6 text-green-600" /></div>
                      <div>
                    ...
                    <div className="flex flex-wrap gap-4 text-sm">
                      <div className="flex items-center gap-2 bg-blue-50 px-3 py-2 rounded-xl"><Clock className="w-4 h-4 text-blue-600" /><span className="text-blue-800 font-medium">{salaEncontrada.horario}</span></div>
                      {duration && distance && (
                        <div className="flex items-center gap-2 bg-orange-50 px-3 py-2 rounded-xl"><Navigation className="w-4 h-4 text-orange-600" /><span className="text-orange-800 font-medium">{duration} ({distance})</span></div>
                      )}
                    </div>
                  </div>
                </div>
